...
resource "aws_subnet" "veena_subnet_1" {
  vpc_id     = "${aws_vpc.veena_vpc.id}" 
...
resource "aws_subnet" "veena_subnet_2" { 
  vpc_id     = "${aws_vpc.veena_vpc.id}"
...
resource "aws_security_group" "veena_sg" {
  name        = "Veena"
  vpc_id      = "${aws_vpc.veena_vpc.id}"
